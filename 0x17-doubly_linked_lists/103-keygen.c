...
int main(int argc, char *argv[])
{
	...
	len = strlen(argv[1]);
	password[0] = string[(len ^ 59) & 63]; //Simply XOR and AND operations over the lenght, with 59 and 63 respectively
	printf("%c\n", password[0]);
	for (i = 0; i < len; i++)
	{
		ops += argv[1][i]; //This is a loop through the username, addign each value and incrementing I
	}
	password[1] = string[(ops ^ 79) & 63]; //Again, XOR and AND with 85 and 63 over the result of the addition
	printf("%c\n", password[1]);
	return (0);
}